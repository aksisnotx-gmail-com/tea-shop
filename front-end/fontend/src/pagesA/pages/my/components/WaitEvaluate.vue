<script setup>
    ...
    const orderList = ref([])
    const getWaitEvaluate = async (currentt = 1) => {
        // const res = await getWaitEvaluateApi(currentt)
        const res = await getOrderByStatusApi(ORDER_STATUS.CONFIRM_RECEIPT, currentt)
    ...
    onReachBottom(async () => {
      ...
      if(currentTotal < pageInfo.total) {
              pageInfo.current++
              await getWaitEvaluate(pageInfo.current)
              uni.hideLoading()
          } else {
              uni.hideLoading()
              uni.showToast({
